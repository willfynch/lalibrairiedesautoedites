...
export const OneBookContainer = ({book}:OneBookContainerProps) => {
    const scrollHandle = (e: any, path:string) => {
        e.preventDefault();
        let id = (e.target as HTMLElement).id;
        let target = document.getElementById(path); //removing extra last - (dash)
        target?.scrollIntoView({ behavior: "smooth", block: "start" }) //scrolling the page
       }
